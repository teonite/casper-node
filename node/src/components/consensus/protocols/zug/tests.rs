...
fn create_message(
    ...
    let signed_msg = create_signed_message(validators, round_id, content, signer);
...
fn create_proposal_message(
    ...
    let echo = create_signed_message(validators, round_id, echo_content, signer);
...
fn zug_faults() {
    ...
    let msg = create_message(&validators, 1, vote(true), &ALICE_SIGNER.clone());
    expect_no_gossip_block_finalized(zug.handle_message(&mut rng, sender, msg, timestamp));
    let msg = create_proposal_message(2, &proposal2, &validators, &ALICE_SIGNER.clone());
    expect_no_gossip_block_finalized(zug.handle_message(&mut rng, sender, msg, timestamp));
    let msg = create_message(&validators, 2, vote(true), &ALICE_SIGNER.clone());
    ...
    let msg = create_message(&validators, 3, vote(true), &BOB_SIGNER.clone());
    expect_no_gossip_block_finalized(zug.handle_message(&mut rng, sender, msg, timestamp));
    let msg = create_message(&validators, 3, vote(false), &BOB_SIGNER.clone());
    ...
    let msg = create_message(&validators, 3, vote(true), &CAROL_SIGNER.clone());
    expect_no_gossip_block_finalized(zug.handle_message(&mut rng, sender, msg, timestamp));
    let msg = create_message(&validators, 3, vote(false), &CAROL_SIGNER.clone());
    let outcomes = zug.handle_message(&mut rng, sender, msg, timestamp);
...
fn zug_sends_sync_request() {
    ...
    let msg = create_proposal_message(0, &proposal0, &validators, &ALICE_SIGNER.clone());
    zug.handle_message(&mut rng, sender, msg, timestamp);
    let msg = create_message(&validators, 0, vote(false), &BOB_SIGNER.clone());
    zug.handle_message(&mut rng, sender, msg, timestamp);
    let msg = create_message(&validators, 0, vote(true), &CAROL_SIGNER.clone());
    zug.handle_message(&mut rng, sender, msg, timestamp);
    let msg = create_message(&validators, 0, vote(false), &CAROL_SIGNER.clone());
    zug.handle_message(&mut rng, sender, msg, timestamp);
...
fn zug_handles_sync_request() {
    ...
    let msg = create_proposal_message(0, &proposal0, &validators, &ALICE_SIGNER.clone());
    zug.handle_message(&mut rng, sender, msg, timestamp);
    let msg = create_message(&validators, 0, echo(hash0), &BOB_SIGNER.clone());
    zug.handle_message(&mut rng, sender, msg, timestamp);
    let msg = create_message(&validators, 0, vote(false), &BOB_SIGNER.clone());
    zug.handle_message(&mut rng, sender, msg, timestamp);
    let msg = create_message(&validators, 0, vote(true), &ALICE_SIGNER.clone());
    zug.handle_message(&mut rng, sender, msg, timestamp);
    let msg = create_message(&validators, 0, vote(true), &CAROL_SIGNER.clone());
    zug.handle_message(&mut rng, sender, msg, timestamp);
    let msg = create_message(&validators, 0, vote(false), &CAROL_SIGNER.clone());
