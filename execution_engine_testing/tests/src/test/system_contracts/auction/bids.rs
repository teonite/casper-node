...
static BID_ACCOUNT_3_PK: Lazy<PublicKey> = Lazy::new(|| {
    let secret_key = SecretKey::ed25519_from_bytes([208; SecretKey::ED25519_LENGTH]).unwrap();
    PublicKey::from(&secret_key)
});
static BID_ACCOUNT_3_ADDR: Lazy<AccountHash> = Lazy::new(|| AccountHash::from(&*BID_ACCOUNT_3_PK));
const BID_ACCOUNT_3_BALANCE: u64 = MINIMUM_ACCOUNT_CREATION_BALANCE;
...
#[ignore]
#[test]
fn should_enforce_max_delegators_per_validator_cap_with_vips() {
    // TODO: .with_max_whitelist_delegators_per_validator(1u32)
    // TODO: make sure max_delegators >= max_whitelist_delegators
    let chainspec = ChainspecConfig::default().with_max_delegators_per_validator(2u32);
    ...
    // TODO: make sure this delegator is not on the whitelist
    let delegation_request_3 = ExecuteRequestBuilder::standard(
        *DELEGATOR_1_ADDR,
        // *BID_ACCOUNT_3_ADDR,
        CONTRACT_DELEGATE,
        runtime_args! {
            ...
            ARG_VALIDATOR => NON_FOUNDER_VALIDATOR_1_PK.clone(),
            ARG_DELEGATOR => DELEGATOR_1.clone(),
            // ARG_DELEGATOR => BID_ACCOUNT_3_PK.clone(),
        },
    )
    .build();
    ...
    // // TODO: make sure this delegator IS on the whitelist
